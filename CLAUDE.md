...
- **`bacio --repo <PREFIX> <subcommand>`** (or `BACIO_REPO=<PREFIX>`) — drive a specific repo *or workspace* instead of resolving one from cwd. A workspace (`repos.kind = 'workspace'`) has no path on disk, so `git.Detect` can never find one: **this selector is the only way to drive a workspace from the CLI.** `repoSelector()` short-circuits `resolveRepoC` (`<worktree_root>/internal/cli/context.go`) *before* `git.Detect`, so it also stops a stray `bacio issue add` auto-registering whatever unrelated git repo you happen to be standing in. It is a lookup, never a create — the prefix must already exist. Prefixes are case-insensitive on input, uppercase in the store. `bacio status` deliberately ignores it (it is a cwd/environment probe).
...
- **Never add a key to an existing sync manifest, and never add a file inside an existing record folder.** `repo.yaml` / `issue.yaml` / `doc.yaml` / `comment.yaml` / `index.yaml` / `redirects.yaml` are all parsed by `strictDecode` (`<worktree_root>/internal/sync/yaml_parse.go`) with `KnownFields(true)`, so an **older** bacio on another machine **hard-fails its entire `bacio sync` run** on an unknown field — and silently strips the key on its next export, because `ExportStaged` diffs staging-vs-target byte-wise. Worse, a new file *inside* an existing record folder (`repos/<P>/docs/<label>/…`), or a nested record folder, makes an older binary `os.RemoveAll` that record and then `propagateDeletes` drop it from the DB **on every machine** — silent, cross-machine data loss. All new synced data goes in a **new top-level sibling** under `repos/<PREFIX>/`: that is why the page tree and the Kanban ship as `workspace.yaml`, `folders/<uuid>/folder.yaml` and `kanban/<uuid>/column.yaml`, and why membership lives on those containers rather than on the member. See `<worktree_root>/docs/background-sync.md`.
