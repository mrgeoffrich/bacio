...
- **Global app settings.** `app_settings(key, value, updated_at)` is the global (not per-repo) sibling of `tui_settings`; helpers in `internal/store/app_settings.go`. Used today for the desktop app's customisable dispatch prompt templates (`prompt_template.<mode>`). Reuse it for any future global preference rather than adding a typed table.
