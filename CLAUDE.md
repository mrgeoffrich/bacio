...
Designed and shipped per [`docs/worktree-environments.md`](docs/worktree-environments.md); CLI surface (`bacio worktree init / show / list / rm`) follows the six agent-CLI principles (`--json`, `--dry-run`, schema entries `worktree.init` / `worktree.rm`). Heavy lifting lives in [`internal/wtenv/`](internal/wtenv/). `bacio worktree rm` also reaps port-bound processes (BACI-93): it terminates any bacio process holding a `LISTEN` socket on the torn-down worktree's API port (`--keep-processes` opts out, `--dry-run` lists the PIDs) so an orphan can't keep holding the shared `ui_leader` lease. Port discovery is the only platform-specific surface — it lives in [`internal/procfind/`](internal/procfind/) (`lsof` on unix, `netstat`/`tasklist` on Windows, build-tag split).
...
Per-job todos don't leak across dispatches: when the PostToolUse hook records a `TaskCreate`, it stamps the row with the session's currently-claimed `issue_key` (resolved from the single open claim at hook time; orphan-bucketed when zero or many claims are open) and (BACI-132) with the active `dispatch_id` for that `(session, issue)` — the newest non-cancelled dispatch targeting the session. The Agents view and the kanban card's `n/m` Tasks pill both filter per-`(session, issue, dispatch)` triple, so even a session that handles two dispatches on the same issue (e.g. plan → implement on BACI-X) shows the current dispatch's rows on each surface — the planning dispatch's already-completed tasks stop merging in once the implement dispatch starts. Pre-BACI-132 rows (and rows the hook couldn't attribute to an in-flight dispatch) carry `dispatch_id IS NULL` and fall out of the per-dispatch card filter; the unfiltered HTTP path (`GET /agents/sessions/{id}/todos` without `dispatch_id`) still returns them. Prior-dispatch rows stay in `agent_session_todos` (queryable per `dispatch_id`), they just stop bleeding into the foreground UI (BACI-62 + BACI-132).

Dispatched workers — and any agent-mode session whose cwd is in the primary checkout — are **confined by a PreToolUse hook** (BACI-116, hardened in BACI-129). `bacio install-agent` wires a sixth `bacio hook` subcommand — `pre-tool-use`, a PreToolUse matcher hook (`matcher: "Write|Edit"`) — that classifies cwd via `git rev-parse --git-dir` vs `--git-common-dir`: inside the **primary** worktree (the main checkout) every `Write`/`Edit` is denied (the BACI-129 main-checkout escape closed), inside a **linked** worktree the call is allowed only if its `file_path` resolves under that worktree's root (the BACI-116 containment check), and outside any git repo the call is allowed (fail-open). The manifest-based precondition is gone — confinement no longer depends on `bacio worktree init` having run. `Bash` is deliberately not confined — confining the write tools keeps the parent checkout clean, so a stray `cd <main> && git commit` has nothing to commit. The handler is fail-open: every error path allows the call. `Bash` cwd does not persist across calls in a subagent, so a worker that `cd`s to the parent repo would otherwise silently edit and commit there — this hook is the enforcement. Belt-and-braces: `model.WorktreeGuardPreamble` (prepended to every generated agent file) mandates that the worker's first `TaskCreate` task be an explicit "establish working directory" step recording the worktree-root prefix, and that it re-run `git branch --show-current` immediately before `git commit` and before `git push`.
