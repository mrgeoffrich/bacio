...
- **Global app settings.** `app_settings(key, value, updated_at)` is the global (not per-repo) sibling of `tui_settings`; helpers in `internal/store/app_settings.go`. Used today for the customisable dispatch prompt templates (`prompt_template.<mode>`) and their state-gates (`prompt_states.<mode>` — the set of issue states a stage's prompt is valid to run from), edited from the desktop Settings screen or `bacio settings template` / `bacio settings template states` (`internal/cli/settings.go`); and for the desktop Board's `board.hide_empty_columns` preference (auto-hide kanban columns with zero cards), edited from the desktop Settings screen via `client.Get/SetBoardPreferences`. Reuse it for any future global preference rather than adding a typed table. Like the agent registry, `app_settings` has no remote analogue in v1 — the `remoteClient` methods return `ErrLocalOnly`.
