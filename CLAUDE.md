...
- **Claims carry a `prompt`; `taken`/`busy` are derived, never stored.** `agent_claims` has a `prompt` column (the instruction the agent claimed from). An issue is `taken` and a session is `busy` iff there's an open (unreleased) claim — `model.SessionBusy` and the `taken` field in issue views compute this; the claim rows are the single source of truth, so nothing can drift. Reuse `agent_claims` (and `store.ListClaimsForIssue` / `OpenClaimsBySession`) for per-issue session history rather than a parallel table.
