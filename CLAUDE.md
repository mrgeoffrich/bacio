...
Dispatched workers are **confined to their git worktree** by a PreToolUse hook (BACI-116). `bacio install-agent` wires a sixth `bacio hook` subcommand — `pre-tool-use`, a PreToolUse matcher hook (`matcher: "Write|Edit"`) — that denies any `Write`/`Edit` whose `file_path` resolves outside the dispatch worktree root (resolved by walking up from the call's `cwd` to a worktree manifest via `wtenv.Resolve`; confinement engages only when a manifest is present, i.e. the dispatched-worker case). `Bash` is deliberately not confined — confining the write tools keeps the parent checkout clean, so a stray `cd <main> && git commit` has nothing to commit. The handler is fail-open: every error path allows the call. `Bash` cwd does not persist across calls in a subagent, so a worker that `cd`s to the parent repo would otherwise silently edit and commit there — this hook is the enforcement. Belt-and-braces: `model.WorktreeGuardPreamble` (prepended to every generated agent file) mandates that the worker's first `TaskCreate` task be an explicit "establish working directory" step recording the worktree-root prefix, and that it re-run `git branch --show-current` immediately before `git commit` and before `git push`.
