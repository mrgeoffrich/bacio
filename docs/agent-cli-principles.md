...
The `install-*` verbs — `bacio install-skill` and `bacio install-agent` (the latter consolidating the former `install-hooks` / `install-channel` / `install-agents` into one command, BACI-79) — are the same kind of shim: they write host config files (`.claude/skills/`, `.claude/settings.json`, `.mcp.json`, `.claude/agents/`) into the local repo rather than mutating bacio state, so they too skip rules #1–#5 (no `--json`, no `bacio schema` entry, no `--dry-run`). Their plan/confirm output before the `--yes`-skippable prompt is the de-facto rehearsal affordance. They are documented in `SKILL.md` (rule #6 holds).
