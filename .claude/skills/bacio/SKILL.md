...
- **Agent cards composite (BACI-50).** `GET /repos/{prefix}/agents/cards` and `GET /agents/cards` (cross-repo) return assembled `AgentCard[]` — sessions enriched with derived busy/waiting flags, open claims with the claimed issue's current state, dispatches bucketed onto each target, and the TodoWrite mirror with done/total counts. Same shape the desktop's Agents view consumes; web bundle uses it directly. CamelCase JSON tags.
