...
- **Push** — if the session runs the `bacio channel` MCP server, dispatches
  ...
  created. `bacio install-agent --yes` registers the channel in the
  repo's `.mcp.json` (alongside the hook + agent-file steps) and prints
  the `claude` launch command (channels are
...
and the imperative button text in one call.

BACI-76: a dispatch template's body is the **system prompt** of a
per-mode custom subagent (`.claude/agents/bacio-<mode>-worker.md`,
generated by `bacio install-agent`). Editing a body via `bacio
settings template set` / the Settings panel updates the canonical row,
but the change does not reach dispatched workers until `bacio
install-agent` regenerates the agent file — re-run it after every body
edit (the CLI prints a reminder; `bacio status` flags stale files).
`{{issue_id}}` / `{{issue_title}}` / `{{repo_prefix}}` interpolation is
a compose-time feature and does **not** survive into an agent file (a
system prompt is fixed per agent type and cannot embed a specific
ticket) — write a dispatch brief to refer to "the ticket named in your
dispatch prompt" instead. `install-agent` warns and skips a user
template whose body still contains `{{`.

The `bacio settings template
...
`bacio install-agent` merges command hooks into the repo's
`.claude/settings.json` (alongside its agent-file and channel steps; it
prints the combined plan and prompts first — pass `--yes` to accept
non-interactively):
...
If the repo has `bacio install-agent` set up, the register / heartbeat /
...
- **CLI verbs with no API equivalent** (touch the local filesystem or terminal): `bacio init` (use `POST /repos`), `bacio install-skill`, `bacio install-agent`, `bacio doc add --from-path` / `--content-file` (inline `content` in the body), `bacio doc export` (use `/download`), `bacio tui`, `bacio hook *`, `bacio channel`. The typed `bacio settings template add/rename/delete/restore-defaults` verbs are still CLI-local, but BACI-50 added matching HTTP routes (see "Prompt-template endpoints" below).
- **Agent registry endpoints.** The dozen register/heartbeat/end/claim/release/list/show/inbox/ack/dispatch/list-dispatches-in-repo/auto-dispatch verbs reach the server under `/repos/{prefix}/agents/sessions` (register, list-in-repo, list-open-claims-in-repo), `/repos/{prefix}/agents/dispatches` (BACI-35: create + list-in-repo), `/repos/{prefix}/issues/{key}/dispatch` (BACI-40: state-gated auto-pick), and `/agents/sessions/{session_id}/...` (heartbeat/end/claim/release/inbox/todos + show), plus `/agents/dispatches/{id}/ack`. `GET /agents/sessions/{session_id}/todos` (BACI-45) returns the session's current task-list mirror (TaskCreate / TaskUpdate; rewired BACI-60) — read-only, writes are hook-driven only. Cross-repo variants of the two lists live at `/agents/sessions` and `/agents/claims/open`. Stub sessions (`registered_at` NULL) are hidden by default on the list endpoints — pass `?all=true` to include them.
- **Prompt-template endpoints (BACI-36, BACI-50).** Bodies and state-gates are global app-settings, not repo-scoped. `GET /settings/templates` returns the resolved body for every dispatch stage (a `mode → body` map); `GET /settings/templates/states` returns the state-gate map. Per-stage mutations live at `PUT/DELETE /settings/templates/{mode}` (body) and `PUT/DELETE /settings/templates/{mode}/states` (state-gate). An empty body/list on PUT is rejected — use DELETE to revert to the built-in default. BACI-50 added the typed CRUD: `GET /settings/templates/full` returns the rich DTO list (slug, name, label, body, allowed_states, defaults, is_builtin); `POST /settings/templates` creates a new template; `POST /settings/templates/{slug}/rename` renames (slug and/or name); `DELETE /settings/templates/{slug}/row` deletes the row (distinct from the body-reset endpoint at `…/{mode}`); `POST /settings/templates/restore-builtins` re-seeds any missing built-ins and returns the post-state. Dry-run + `X-Dry-Run` header work everywhere.
- **Agent cards composite (BACI-50).** `GET /repos/{prefix}/agents/cards` and `GET /agents/cards` (cross-repo) return assembled `AgentCard[]` — sessions enriched with derived busy/waiting flags, open claims with the claimed issue's current state, dispatches bucketed onto each target, and the task-list mirror with done/total counts. Same shape the desktop's Agents view consumes; web bundle uses it directly. CamelCase JSON tags.
- **Leader (BACI-50).** `GET /leader` returns `{"amLeader": <bool>, "holderLabel": "<string>"}` — `bacio api` runs the UI leader elector alongside the desktop and TUI (all racing on the same `ui_leader` row). Heartbeats every 10s. Auth-gated like `/version`.
- **Board preferences (BACI-47/D).** One scalar global flag today, `board.hide_empty_columns`. `GET /settings/board-preferences` returns `{"hide_empty_columns": <bool>}`; `PUT /settings/board-preferences` with the same shape persists it (audited as `board_pref.update`). Unknown fields are rejected; `?dry_run=1` / `X-Dry-Run` are honoured. Auth-gated like everything else.
- **`/version` (BACI-47/A).** `GET /version` returns `{"version": "<internal/version.String()>"}` — the same string `bacio --version` prints and the Agents panel reports per session, so a single readout can spot stale channels. Requires auth when `--token` is set (NOT in the `/healthz` public bypass).
...
To set the repo up for agent-driven bacio work — automatic session
registration, dispatch delivery, and the per-mode dispatch subagents —
run the single consolidated installer:

```bash
bacio install-agent --yes
```

`install-agent` performs three steps in one invocation, each
non-destructive and idempotent:

1. **Subagent files** — writes `.claude/agents/bacio-<mode>-worker.md`,
   one custom subagent per dispatchable prompt template (`design`,
   `plan`, `implement`, `review`, `ship`, `fix_review`, plus any
   user-created templates). Each file's body is the template's brief,
   which becomes that subagent's system prompt; the bacio channel's
   dispatch payload then only names which subagent to spawn instead of
   pasting the whole brief.
2. **Hooks** — merges the five `bacio hook` command hooks into
   `.claude/settings.json` (existing non-bacio hooks are preserved).
   See "Hook integration" above for what each hook does.
3. **Channel** — merges a `bacio` entry into the repo's `.mcp.json` for
   real-time (push) dispatch delivery, and prints the recommended
   launch command —
   `BACIO_AGENT_MODE=1 claude --dangerously-skip-permissions --dangerously-load-development-channels server:bacio`.

One combined plan is printed; pass `--yes` (`-y`) to accept all three
steps automatically, which is required when running non-interactively.
Re-run `bacio install-agent` after editing a template body (via `bacio
settings template set` or the Settings panel) to regenerate the
subagent files — until then dispatched workers use the previous body.
`bacio status` reports per-template agent-file freshness (`up-to-date` /
`missing` / `stale`).
