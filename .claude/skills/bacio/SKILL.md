...
generated by `bacio install-agents`). Editing a body via `bacio
...
install-agents` regenerates the agent file — re-run it after every body
...
dispatch prompt" instead. `install-agents` warns and skips a user
...
Finally, generate the per-mode dispatch subagent files (BACI-76):

```bash
bacio install-agents --yes
```

It writes `.claude/agents/bacio-<mode>-worker.md` — one custom subagent
per dispatchable prompt template (`design`, `plan`, `implement`,
`review`, `ship`, `fix_review`, plus any user-created templates). Each
file's body is the template's brief, which becomes that subagent's
system prompt; the bacio channel's dispatch payload then only names
which subagent to spawn instead of pasting the whole brief. Same
plan/confirm + `--yes` behaviour as the other installers. Re-run
`bacio install-agents` after editing a template body (via `bacio
settings template set` or the Settings panel) to regenerate the files —
until then dispatched workers use the previous body. `bacio status`
reports per-template agent-file freshness (`up-to-date` / `missing` /
`stale`).
