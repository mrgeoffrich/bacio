...
- **`bacio issue` Pipeline verbs** — the Pipeline page's ordering + process controls: `bacio issue reorder <KEY> <position>` moves a card within its Backlog (`todo`) / Shipping (`to_be_shipped`) ordering band (position 1 = top, next to go); `bacio issue process set <KEY> <preset>` assigns a preset job chain to an `in_pipeline` card (presets: `plan-implement-ship`, `implement-ship`, `plan-implement`, `plan`, `implement`); `bacio issue process edit <KEY> <mode…>` (BACI-294) edits the **pending tail** of an already-started chain — `stages` is the re-ordered list of pending job modes that replaces the card's pending jobs, while the completed/running/cancelled jobs are kept verbatim as a locked prefix and the tail is re-sequenced after them (ship may only be the final stage; duplicate modes are allowed, e.g. `implement review implement`); `bacio issue ship <KEY>` hands an `in_pipeline` card off to the Shipping column (dispatches no agent — the ship agent fires from Shipping); `bacio issue auto-ship <on|off>` toggles the per-repo Shipping auto-ship. The per-card engine controls (manual Start / Stop, the Auto drive-mode toggle) are API/engine-only (no CLI verb): the controller job-engine owns chain progression for `in_pipeline` cards, queuing ordinary dispatches the matcher binds and advancing state itself. All five verbs take `--json` / `--dry-run` and have `bacio schema` entries (`issue.reorder`, `issue.process.set`, `issue.process.edit`, `issue.ship`, `issue.auto-ship`).
...
- **`bacio agent`** — the local agent-session registry: register/heartbeat/end, claim/release, dispatch/inbox/ack/cancel, list/show, and `agent questions` for user clarifications. Never synced. `bacio agent queue-followon <KEY> --mode <stage>` (BACI-180) attaches a dormant follow-on dispatch to an issue's in-flight (parent) dispatch — the matcher ignores the row until the parent acks or is cancelled, then a controller sweep promotes it and the next free worker picks it up. Use this to chain `plan → implement` (or any sibling stage) without babysitting the release. BACI-217: the same verb also accepts a **blocked-and-idle** card (no in-flight dispatch, at least one open `blocks` edge pointing at it) and queues a `gate=blockers` variant instead — the dormant row fires the moment every blocker reaches `done` or `cancelled`. The two variants are mutually exclusive on a single row, so a card that is both blocked and has an in-flight parent gets the parent-acks variant. `bacio agent cancel-followon <KEY>` removes the dormant row regardless of variant (idempotent — no-op when nothing is queued). Single-slot per issue across both variants. `bacio agent dispatch-chain <KEY> --mode <primary> --then <follow-on>` (BACI-209) is the compound variant: queues the primary dispatch AND the dormant follow-on against the brand-new parent in one transaction — use it when you want both stages locked in up front (e.g. `plan` then `implement` on a fresh `todo` card) rather than dispatching the primary and attaching a follow-on as a second step. Schema: `agent.dispatch-chain`.
...
- **`bacio agent cancel-followon` only matches dormant rows** (BACI-180). Once the BACI-179 controller sweep has promoted a follow-on (predecessor settled, no claim races in), the row is a regular queued dispatch — no longer reachable through `cancel-followon`. Reach for `bacio agent cancel <id>` against the promoted dispatch's id instead. The follow-on verbs are also single-slot per issue: a second `queue-followon` against the same issue while a dormant follow-on already exists is rejected.
