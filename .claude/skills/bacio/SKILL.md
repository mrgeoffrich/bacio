...
- **`bacio issue` Pipeline verbs** — the Pipeline page's ordering + process controls: `bacio issue reorder <KEY> <position>` moves a card within its Backlog (`todo`) / Shipping (`to_be_shipped`) ordering band (position 1 = top, next to go); `bacio issue process set <KEY> <preset>` assigns a job chain to an `in_pipeline` card — the flag/positional path takes a preset slug (presets: `plan-implement-ship`, `implement-ship`, `plan-implement`, `plan`, `plan_large`, `design`, `implement`), and the `--json` path additionally accepts an explicit ordered stage list via `"stages"` (e.g. `["design","plan_large","implement","ship"]`, mutually exclusive with `"process"`) for arbitrary chains the presets don't enumerate; `bacio issue process edit <KEY> <mode…>` (BACI-294) edits the **pending tail** of an already-started chain — `stages` is the re-ordered list of pending job modes that replaces the card's pending jobs, while the completed/running/cancelled jobs are kept verbatim as a locked prefix and the tail is re-sequenced after them (ship may only be the final stage; duplicate modes are allowed, e.g. `implement review implement`); `bacio issue process reset <KEY>` (BACI-314) wipes an `in_pipeline` card's **entire** job chain — including completed/cancelled history — so the card drops back to the from-scratch process picker (the deliberate counterpart to `process set`, which refuses once a job has started; `reset` is refused while a job is running — stop it first); `bacio issue ship <KEY>` hands an `in_pipeline` card off to the Shipping column (dispatches no agent — the ship agent fires from Shipping); `bacio issue auto-ship <on|off>` toggles the per-repo Shipping auto-ship. The per-card engine controls (manual Start / Stop, the Auto drive-mode toggle) are API/engine-only (no CLI verb): the controller job-engine owns chain progression for `in_pipeline` cards, queuing ordinary dispatches the matcher binds and advancing state itself. All six verbs take `--json` / `--dry-run` and have `bacio schema` entries (`issue.reorder`, `issue.process.set`, `issue.process.edit`, `issue.process.reset`, `issue.ship`, `issue.auto-ship`).
...
- **`bacio proxy`** — inspect the BACI-302 reverse-proxy capture index. `bacio proxy stats` (BACI-303) rolls the per-request `proxy_requests` rows up into per-FQDN statistics — request count, bytes in/out, error rate, p50/p95 round-trip latency, first/last seen — busiest host first; `--since 1h` windows the lookback, `--limit N` caps the rows folded into the rollup. `bacio proxy capture <id>` (BACI-306) shows the parsed detail of one captured Anthropic SSE turn (reconstructed text / thinking / tool_use blocks + merged token usage), keyed on the `proxy_requests` id; `bacio proxy job <dispatch_id>` (BACI-306) assembles a dispatch's captured turns into one ordered message transcript (primary thread + summed usage + auxiliary title-gen / structured-output probes). `bacio proxy jobs` (BACI-322) is the transcript-browser list — one summary row per distinct dispatch that has parsed captures (turn count, summed token usage, the primary thread's model, last-seen, plus the dispatch's issue key / mode / agent); scope with `--repo <PREFIX>` (the active repo), `--issue <KEY>`, `--mode <m>`, and `--since`. `bacio proxy grep <text>` (alias `search`, BACI-320) is the *content* filter complementing `proxy captures`' index filter — a case-insensitive substring search over the parsed message bodies that emits one match line per matching content block (each carrying the `CAPTURE` id you then feed to `proxy capture <id>` / `proxy raw <id>`, closing the search→drill-in loop); scope with `--role assistant|user`, `--block text|thinking|tool_use|tool_result`, the `--dispatch` / `--session` / `--agent` correlation, `--since`, and `--limit`. The capture index is cross-cutting (no repo column — `proxy jobs` derives its `--repo` filter from each dispatch's repo), so they run anywhere. The read verbs are read-only like `bacio history` / `bacio status` — no `--json` / `--dry-run` / `bacio schema` entry (prefer `-o json` for the full structured shape). The one **mutating** verb is `bacio proxy reparse` (BACI-321) — the manual escape hatch over the leader-gated controller sweep that backfills `proxy_messages` from captured Anthropic traffic the live recorder path missed (a full hand-off queue, a transient parse error, a window where parsing was off), so those turns show up in `bacio proxy job`. No flags = sweep every eligible dispatch; `--dispatch <id>` scopes to one job. It follows the six agent-CLI rules (`--json` in, a `proxy.reparse` schema entry, `--dry-run` to project counts without writing); v1 only backfills fully-unparsed dispatches (non-destructive), and the destructive partial-gap rebuild (`--rebuild`) is reserved but not implemented.
...
- **`bacio settings`** — global settings: `template` (dispatch prompt templates), `show-archived`, `sync-background`, `archive` (BACI-162 auto-archive toggle + retention window), `timezone` (BACI-312 — `bacio settings timezone [IANA-ZONE]` gets/sets the global `ui.timezone`, e.g. `Australia/Sydney`; drives the Pipeline Shipping-column Shipped pill's local-midnight "Today" cutoff. Unset by default — the desktop / web UI auto-detects the browser zone and persists it on first run; the server stays timezone-agnostic). One verb in this group is **per-repo, not global**: `bacio settings default-feature [SLUG]` (BACI-235) sets the feature `bacio issue add` (and every other surface — REST, TUI new-issue, web composer) auto-applies when no explicit `feature_slug` is provided. Empty slug / `--clear` / `--json '{"slug":""}'` unsets it; the FK on the stored column is ON DELETE SET NULL so deleting the referenced feature auto-clears the setting. An explicit `feature_slug` always overrides the default.
...
- **Harness shims** — `bacio tui` (terminal kanban), `bacio api` / `bacio web` (HTTP API ± embedded UI), `bacio hook` / `bacio channel` (Claude Code integration), `bacio agent-run-command` (BACI-218 — prints the one-liner that spins up an agent-mode Claude session, designed for `eval "$(bacio agent-run-command)"` / `alias bacio-agent="$(bacio agent-run-command)"`). These take no `--json` / `--dry-run` and have no `bacio schema` entry. `bacio install-agent` wires a PostToolUse `bacio hook set-title` entry (matcher `mcp__bacio__register`) alongside the existing task-list mirror so a dispatched worker's terminal title flips to its agent slug as soon as `register` completes (BACI-147). It also wires a `bacio hook stop-failure` entry on the Claude Code `StopFailure` event (BACI-296) — when a turn dies on an Anthropic API error it records the errored state on the agent session (surfaced as a red `errored` liveness) and reconciles the worker's in-flight Pipeline job — both transient outages and terminal auth/billing errors pause the chain in place (the card stays `in_pipeline` with a distinct `engine_pause_reason`; BACI-300 retired the old `needs_action` hand-off). The `bacio channel` MCP server exposes five tools to a running agent: `reply` (ack a dispatch), `register` (complete session registration), `ask_user_question` (BACI-53 — **blocking** multi-choice clarification), `send_user_notification` (BACI-287 — **non-blocking, fire-and-forget** agent→user notification into the notification bell + TUI Notifications tab; optional `issue_id` deep-links it to a ticket), and `report_subagent_incomplete` (BACI-328 — the supervisor calls it when a `Task` subagent ended early/interrupted — a user soft-cancel — so bacio reconciles the dirty in-flight Pipeline job: cancel the running job + dispatch, neutral `subagent_cancelled` halt, release the stale claim. It **replaces** `reply` for that dispatch — call one or the other, never both, since acking a cancelled dispatch errors). Reach for `send_user_notification` when the user should *see* something and you carry on; reach for `ask_user_question` when you need them to *answer*. (BACI-307 retired the `attach_transcript` tool — captured Anthropic traffic now lives in `proxy_messages`, read via `bacio proxy job <dispatch_id>`.)
