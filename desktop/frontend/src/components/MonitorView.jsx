...
// Monitor view defines its own local const the same way other views do).
const POLL_INTERVAL_MS = 10_000;

// COLUMNS drives both the sortable header and the per-row cells. `key` is
// the ProxyFQDNStat field; `sortValue` extracts the value to sort on;
...
// MonitorView is the desktop / web reverse-proxy traffic monitor — a
// read-only sortable table over the BACI-303 per-FQDN rollup of
// proxy_requests. The proxy_requests table is cross-cutting (no repo_id),
// so the stats are global; `activeBoard` is accepted for nav-uniformity
// but ignored for the data fetch. The view auto-refreshes every 10s while
// mounted (silent — no loading flicker on the poll).
export default function MonitorView() {
  const [stats, setStats] = useState([]);
  const [loading, setLoading] = useState(true);
  const [scope, setScope] = useState('day'); // default Last 24h
