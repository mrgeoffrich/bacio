...
// filtering. BACI-361 moved its `cards` from a prop to useCards(), so the
// suite mocks the CardsProvider hook (a hoisted holder lets each test seed
// the fixture) rather than wrapping the full provider chain.
const hoisted = vi.hoisted(() => ({
  cards: [] as { key: string; title: string; column: string; columnLabel: string }[],
}));
vi.mock('../../state/CardsProvider', () => ({
  useCards: () => ({ cards: hoisted.cards }),
}));

// Only the four fields the component reads are populated; the cast keeps the
...
describe('CommandPalette', () => {
  ...
  it('renders nothing when closed', () => {
    render(<CommandPalette open={false} onClose={vi.fn()} onPick={vi.fn()} />);
