...
var Registry = []Entry{
	...
	{"agent.queue-followon", "Queue a dormant follow-on dispatch (BACI-180) against an issue's in-flight (parent) dispatch. Re-checks the mode's state-gate, resolves the parent via WaitingDispatchForIssue, and fails when there is no open dispatch on the issue. Single-slot per issue: a second call against the same issue while a dormant follow-on already exists is rejected.", typeOf[inputs.AgentQueueFollowOnInput](), inputs.ExampleAgentQueueFollowOn},
	{"agent.cancel-followon", "Cancel the dormant follow-on dispatch attached to an issue (BACI-180). Idempotent: returns the cancelled row when one flipped, or a null projection when there was nothing to cancel.", typeOf[inputs.AgentCancelFollowOnInput](), inputs.ExampleAgentCancelFollowOn},
	{"agent.dispatch-chain", "Queue a primary dispatch + a dormant follow-on against an issue in one transaction (BACI-209). Same shape as `bacio agent dispatch` for the primary (state-gated auto-pick, target-less queued row); the follow-on rides as a dormant queued row linked to the brand-new parent. Single-slot per issue; the follow-on's state-gate is re-evaluated by the controller's promote sweep at fire time, not here.", typeOf[inputs.AgentDispatchChainInput](), inputs.ExampleAgentDispatchChain},
	...
	{"feature.state", "Set a feature's state (active|done|cancelled) — BACI-199. Sets state_manual=1 so the leader-elected archive-sweep's auto-completion pass leaves the row alone until the user pins a new value. Archive (archived_at) stays orthogonal — a `done` or `cancelled` feature is still visible by default until explicitly archived.", typeOf[inputs.FeatureStateInput](), inputs.ExampleFeatureState},
	...
	{"settings.shipped-sfx", "Toggle the BACI-240 ui.shipped_sfx global setting. When on, the topbar Shipped pill plays a short ka-ching SFX on every genuine ship (gated by `prefers-reduced-motion` and the autoplay policy — silently no-ops if either denies). Default OFF — audio is opt-in in a desktop dev tool.", typeOf[inputs.SettingsShippedSfxInput](), inputs.ExampleSettingsShippedSfx},
