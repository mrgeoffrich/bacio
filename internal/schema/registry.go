...
var Registry = []Entry{
	...
	{"issue.process.set", "Assign a process (job chain) to an in_pipeline card (Pipeline). Pass either a preset slug via process (e.g. plan-implement-ship, scope-shelve) OR an explicit ordered stage list via stages (e.g. [\"design\",\"plan_large\",\"implement\",\"ship\"]) — exactly one, they are mutually exclusive. ship (hand off to Shipping) and shelve (return the card to Backlog) are terminal sentinels — each may appear only as the final stage.", typeOf[inputs.IssueProcessInput](), inputs.ExampleIssueProcess},
	{"issue.process.edit", "Edit the PENDING TAIL of an in_pipeline card's job chain (Pipeline, BACI-294). stages is the re-ordered ordered list of pending job modes that replaces the card's pending jobs; the completed/running/cancelled jobs are kept verbatim as a locked prefix and the tail is re-sequenced after them. A terminal sentinel (ship hand-off to Shipping, or shelve demote to Backlog) may appear only as the final stage of the whole chain; duplicate modes are allowed (e.g. implement → review → implement).", typeOf[inputs.IssueProcessEditInput](), inputs.ExampleIssueProcessEdit},
	{"issue.process.reset", "Wipe an in_pipeline card's ENTIRE job chain (Pipeline, BACI-314) — including completed/cancelled history — so the card drops back to the from-scratch process picker. The deliberate counterpart to process set, which refuses once a job has started. Refused while a job is running (stop it first).", typeOf[inputs.IssueProcessResetInput](), inputs.ExampleIssueProcessReset},
	...
	{"issue.mark-done", "Close an in_pipeline card out as done directly (Pipeline, BACI-352). Moves the card straight to done, bypassing the Shipping column and the ship agent — the direct-done counterpart to issue ship. The card's chain must end in the mark_done terminal sentinel (set via process set / edit).", typeOf[inputs.IssueMarkDoneInput](), inputs.ExampleIssueMarkDone},
	...
	{"feature.comment.add", "Add a comment to a feature — the BACI-124 chronological-handoff scratchpad implement-mode workers post to on close-out. The optional `kind` (BACI-333) is 'note' (default — a hand-typed comment, never gated) or 'handoff' (a worker close-out note, dropped without error by the store backstop when the feature has handoffs disabled via `bacio feature handoffs <slug> off`).", typeOf[inputs.FeatureCommentAddInput](), inputs.ExampleFeatureCommentAdd},
	...
	{"feature.handoffs", "Toggle whether worker close-outs append handoff comments to a feature (BACI-333). ON (the default) collects handoffs — sibling workers inherit each other's context on a cohesive feature. OFF stops a standing bucket like `bugs` or `maintenance`, whose unrelated children make handoffs pure noise, from accumulating them: the store backstop drops a kind='handoff' write without erroring the worker, while a hand-typed kind='note' always goes through. Writes only the collect_handoffs column.", typeOf[inputs.FeatureHandoffsInput](), inputs.ExampleFeatureHandoffs},
	...
	{"proxy.reparse", "Manually trigger the BACI-321 proxy_messages backfill — the escape hatch over the leader-gated sweep that reparses dispatch-correlated Anthropic captures the live recorder path missed (a full hand-off queue, a transient parse error, a window where parsing was off) into proxy_messages so their turns appear in the per-job transcript. No fields = sweep every eligible dispatch; `dispatch` scopes to one job's captures. `retry_failed` (BACI-323) clears parse_failed_at on the still-unparsed captures in scope first, so dispatches the parser previously gave up on backfill once the parser bug is fixed (without it those captures stay skipped forever). `rebuild` (BACI-325) destructively rebuilds ONE dispatch: it deletes that dispatch's proxy_messages rows and replays ALL its captures through the current parser, re-classifying every turn — the recovery for a dispatch misclassified by a since-fixed parser bug. `rebuild` REQUIRES `dispatch`; a global rebuild (rebuild with no dispatch) errors cleanly, as an unbounded destructive write over the shared store is out of scope. The non-rebuild backfill only touches fully-unparsed dispatches (non-destructive). Idempotent. Returns {dispatches_scanned, captures_reparsed, captures_failed}.", typeOf[inputs.ProxyReparseInput](), inputs.ExampleProxyReparse},
	...
	{"settings.timezone", "Get or set the BACI-312 ui.timezone global setting — the user's IANA zone name (e.g. Australia/Sydney). Empty body reads the current value; a non-empty `timezone` writes it. The value drives the browser-side local-midnight cutoff for the Pipeline Shipping-column Shipped pill's \"Today\" scope (the server stays timezone-agnostic). Unset by default — the desktop / web UI auto-detects the browser zone and persists it on first run.", typeOf[inputs.SettingsTimezoneInput](), inputs.ExampleSettingsTimezone},
