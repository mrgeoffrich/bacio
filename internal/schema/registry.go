...
var Registry = []Entry{
	...
	{"comment.add", "Add a comment to an issue. Set `eval: true` to mark it as a quality-review note (BACI-131) — the server pins the in-flight (agent_session_id, dispatch_id, mode) snapshot onto the row at write time. Set `transcript_event_ref` (BACI-141) to anchor an eval comment to a specific event inside a `.jsonl` transcript (shape: `tool_use_id:<id>` or `line_index:<n>`).", typeOf[inputs.CommentAddInput](), inputs.ExampleCommentAdd},
