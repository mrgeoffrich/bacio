...
var Registry = []Entry{
	...
	{"settings.shipped-sfx", "Toggle the BACI-240 ui.shipped_sfx global setting. When on, the topbar Shipped pill plays a short ka-ching SFX on every genuine ship (silently no-ops under the browser autoplay policy until a user gesture lands). Default ON (BACI-295); `prefers-reduced-motion` no longer mutes it — that preference governs animation, not audio.", typeOf[inputs.SettingsShippedSfxInput](), inputs.ExampleSettingsShippedSfx},
