...
// captures (the dispatch_id the capture correlates on). Rebuild requests the
// destructive partial-gap rebuild (delete-from-gap-onward + replay); it is
// reserved on the surface but not implemented in v1 — passing it errors cleanly
// with a "not yet implemented" message, so the flag is wired without shipping the
// destructive path.
