...
var builtinTemplateSlugs = []string{
	...
	BuiltinTemplateScope, BuiltinTemplateResearch, BuiltinTemplatePlan, BuiltinTemplatePlanLarge, BuiltinTemplateDesign, BuiltinTemplateImplement,
...
// SubagentTypeForTemplate maps a template slug to the custom subagent
// type `bacio install-agent` generates for it (BACI-76). The result is
// also the basename of the generated `.claude/agents/<name>.md` file —
// the agent file's `name:` frontmatter must equal its basename, and the
// supervisor passes this value as `subagent_type` in its Task call.
//
// Template slugs match `^[a-z0-9_][a-z0-9_-]*$`; the only character
// illegal in a Claude Code agent name ([a-z0-9-]) is the underscore, so
// normalisation is a total underscore→hyphen replacement. A leading
// underscore (the reserved-slug convention, e.g. `_dispatch_preamble`)
// becomes a leading hyphen — harmless, and reserved slugs are never
// passed here (`install-agent` skips `_dispatch_preamble`).
func SubagentTypeForTemplate(slug string) string {
	return "bacio-" + strings.ReplaceAll(slug, "_", "-") + "-worker"
}
...
// DispatchStub is the tiny per-dispatch variable block BACI-76 puts in
// the dispatch payload in place of the full per-mode brief. The durable
// brief now lives in the per-mode subagent's system prompt
// (`.claude/agents/bacio-<mode>-worker.md`, generated by
// `bacio install-agent`); the payload only has to tell the supervisor
// *which* subagent to spawn and *which* ticket it is for. The dispatch
...
// shape the supervisor session receives it (BACI-76 shape):
//
//   - The optional preamble (the rewritten BACI-52 wrapper instructing
//     the parent session to spawn the per-mode custom subagent).
//   - The DispatchStub — a few lines naming the ticket, mode, and
//     subagent type. The durable per-mode brief is NOT here any more;
//     it is the subagent's own system prompt.
//   - An optional free-form note after a blank line.
//
// Each part is independently optional:
//
//   - preamble == "": skip the preamble (the channel's setup-register
//     dispatch and note-only dispatches carry no delegatable work).
//   - stub has no Mode (untyped dispatch): payload is just the note —
//     the supervisor isn't being handed a typed job to delegate.
//   - note == "": payload is preamble + stub, no trailing freeform line.
func ComposeDispatchPayload(preamble string, stub DispatchStub, note string) string {
	preamble = strings.TrimSpace(preamble)
	stubText := stub.render()
	note = strings.TrimSpace(note)

	if stub.Mode == "" {
		// No mode = no typed work brief to delegate = no preamble.
