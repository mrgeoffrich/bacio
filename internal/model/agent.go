...
// type `bacio install-agents` generates for it (BACI-76). The result is
...
// passed here (`install-agents` skips `_dispatch_preamble`).
...
// `bacio install-agents`); the payload only has to tell the supervisor
