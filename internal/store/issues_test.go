...
func TestTerminalAtLifecycle(t *testing.T) {
	...
	// Move done → in_review (reopen). terminal_at must clear.
	if err := s.SetIssueState(open.ID, model.StateInReview); err != nil {
	...
	newTitle := "renamed"
	if err := s.UpdateIssue(closed.ID, &newTitle, nil, nil, nil, nil); err != nil {
