...
- **`bacio feature`** — add/list/show/edit/rm; `feature plan <slug>` prints open issues in dependency order. Features carry an optional single-glyph `emoji` field (BACI-172) that the kanban / desktop renders top-left on every card under the feature; the `plan_large` agent picks one on creation, and users can change it from the desktop Features pane.
