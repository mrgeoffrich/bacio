...
- **`bacio archive`** — `archive sweep` runs the auto-archive passes on demand, including the BACI-199 feature auto-completion pass that promotes `active`-with-every-child-terminal features to `done` / `cancelled` (without overriding the sticky bit set by `bacio feature state`).
...
- **`bacio settings`** — global settings: `template` (dispatch prompt templates), `show-archived`, `sync-background`, `archive` (BACI-162 auto-archive toggle + retention window).
