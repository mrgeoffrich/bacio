...
- **`bacio agent`** — the local agent-session registry: register/heartbeat/end, claim/release, dispatch/inbox/ack/cancel, list/show, and `agent questions` for user clarifications. Never synced. `bacio agent queue-followon <KEY> --mode <stage>` (BACI-180) attaches a dormant follow-on dispatch to an issue's in-flight (parent) dispatch — the matcher ignores the row until the parent acks or is cancelled, then a controller sweep promotes it and the next free worker picks it up. Use this to chain `plan → implement` (or any sibling stage) without babysitting the release. `bacio agent cancel-followon <KEY>` removes the dormant row (idempotent — no-op when nothing is queued). Single-slot per issue. `bacio agent dispatch-chain <KEY> --mode <primary> --then <follow-on>` (BACI-209) is the compound variant: queues the primary dispatch AND the dormant follow-on against the brand-new parent in one transaction — use it when you want both stages locked in up front (e.g. `plan` then `implement` on a fresh `todo` card) rather than dispatching the primary and attaching a follow-on as a second step. Schema: `agent.dispatch-chain`.
