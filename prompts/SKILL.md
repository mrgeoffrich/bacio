...
- **`bacio issue` Pipeline verbs** — the Pipeline page's ordering + process controls: `bacio issue reorder <KEY> <position>` moves a card within its Backlog (`todo`) / Shipping (`to_be_shipped`) ordering band (position 1 = top, next to go); `bacio issue process set <KEY> <preset>` assigns a preset job chain to an `in_pipeline` card (presets: `plan-implement-ship`, `implement-ship`, `plan-implement`, `plan`, `implement`); `bacio issue ship <KEY>` hands an `in_pipeline` card off to the Shipping column (dispatches no agent — the ship agent fires from Shipping); `bacio issue auto-ship <on|off>` toggles the per-repo Shipping auto-ship. The per-card engine controls (manual Start / Stop, the Auto drive-mode toggle) are API/engine-only (no CLI verb): the controller job-engine owns chain progression for `in_pipeline` cards, queuing ordinary dispatches the matcher binds and advancing state itself. All four verbs take `--json` / `--dry-run` and have `bacio schema` entries (`issue.reorder`, `issue.process.set`, `issue.ship`, `issue.auto-ship`).
