...
- **`bacio comment`** — add/list/rm. Pass `--eval` (or `"eval": true` on the JSON path) on `bacio comment add` to mark the row as a BACI-131 quality-review note — the server pins the in-flight `(agent_session_id, dispatch_id, mode)` snapshot onto the comment at write time, and `bacio comment list -o json` surfaces the four fields on every row. BACI-141 widens the eval shape further: pass `"transcript_event_ref": "tool_use_id:<id>"` or `"line_index:<n>"` on `comment add --json` to anchor the note to a specific event inside a `.jsonl` transcript (the transcript viewer's per-event composer fills this for you; empty / omitted keeps the comment dispatch-level anchored).
